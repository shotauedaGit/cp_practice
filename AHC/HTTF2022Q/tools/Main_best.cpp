...
class HTTF2022Q{
    ...
    vector<int> doable_task(){
        ...
        if(errPrint)cerr<<"day:"<<today<<" doable: "<<ret.size()<<endl;
        return ret;
    }
    ...
    void change_si_random(int si){
        ...
        int rnd_k = rand()%K;
        while(Timer_end()<(2.7f*task_done_cnt)+80){
            rnd_k = rand()%K;
            change_skill(si,rnd_k, 0.5f ,true);
        }
    }
    void change_skill(int si,int k,float dx,bool isFirst){
        vector<float> si_1 = s[si];
        // modify
        si_1[k] += dx; chmax(si_1[k],0.0f);
