...
fn exec(command: &str, args: &Vec<String>) {
	...
	for d in 1.. {
		...
		loop {
			...
			//eprintln!("read input. day {}",d);
			stdout.read_line(&mut out).unwrap_or_else(|_| err());
			//eprintln!("input has been read. day {}",d);
			...
			if ss.len() == 0 {
				eprintln!("[ss.len == 0]Illegal output format on day {}", d);
				eprintln!("Score = 0");
				std::process::exit(1)
			}
			if let Ok(m) = ss[0].parse::<usize>() {
				if m * 2 != ss.len() - 1 {
					eprintln!("args isnot m*2");
					err2();
				}
				for i in 0..m {
					if let (Ok(a), Ok(b)) = (ss[1 + i * 2].parse::<usize>(), ss[2 + i * 2].parse::<usize>()) {
						...
						eprintln!("Failed to parse {} th a or b",i);
						err2();
					}
				}
			} else {
				eprintln!("Failed to parse m");
				err2();
		...
		if n == input.N || d == MAX_D {
			...
			/*
			let f = std::fs::File::open("score.txt");
			{
				let mut writer = std::io::BufWriter::new(f);
				writer.writeln("{}", score);
			}
			*/
