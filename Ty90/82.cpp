...
class mInt{
    ...
    mInt &operator/=(const ll p){
        mInt<mod> mp(p);
        x = (int)((ll)x*(mp.pow(-1)).x%mod);
         return *this;
    }
...
ll sumFrmTo(ll frm,ll to){
    mInt<MOD> ret(to); ret += frm;
    ret *= (to - frm + 1); ret /= 2;
    return ret.x;
}
...
int main(){
    bool flag=false;
    ll ans=0,sum=0;
    ll L,R;int l,r;cin>>L>>R;

    l = to_string(L).size();
    r = to_string(R).size();
    db2(l,r);ln;

    mInt<MOD> m10(10),m1(1),mAns(0);
    mInt<MOD> mAdd(0);

    vector<ll> d; d.emplace_back(1);
    for(int i=1;i<19;++i){ d.emplace_back( d[i-1]*10 ); }
    ...
    if(l == r){
        ...
        mAdd = l;
        mAdd *= sumFrmTo(L,d[l]-1);
        mAns += mAdd;

        for(ll i=l+1;i<r;++i){
            mAdd = i;
            mAdd *= sumFrmTo(d[i-1],d[i]-1);
            mAns += mAdd;
        }

        mAdd = r;
        mAdd *= sumFrmTo(d[r-1],R);
        mAns += mAdd;
    }
