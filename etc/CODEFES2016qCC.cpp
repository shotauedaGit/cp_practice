...
int main(){
    ...
    if(a[n-1]<lw || hi<a[n-1]){cout<<0<<endl;return 0;}
    range[n-1] = P(a[n-1],a[n-1]);
    ...
    for(int i = n-2; i >= 0; --i){
        ...
        if(a[i] > a[i+1]){
            if(a[i]<lw || hi<a[i]){cout<<0<<endl;return 0;}
            range[i] = P(a[i],a[i]);
        }else{
            if(a[i] < lw){cout<<0<<endl;return 0;}
            range[i] = P(lw,min(hi,a[i]));
        }
